...
```text
早上：
1、处理cvm修改项目id后，云监控没有收到变化的问题，---排查后是因为没有订阅标签团队的南京地域数据
2、处理tce31011的amp_for_cloud 的动态时区问题  --- 目前不支持M13地区的部署，正在找人处理


下午：
1、周会
2、确认支持动态时区的环境信息
3、和测试一起验证tce3100的告警问题，发现出现了多次相同告警 -- 自测未复现，但是测试测会复现
```
...
```text
早上：
1、分析kafka非专业版会绑定kafka实例，导致页面回显失败的问题；问题是缺少维度，需要查看通过维度服务是否可以正常支持来区分kafka的专业版和非专业版
2、分析tce3100支持告警等级的各种不正常情况

下午：
1、排查出现多次告警的原因，这个原因是因为一分钟粒度的触发时间间隔太小，小于第一次处理的时间，导致第二次会插入到告警历史中
2、梳理排查思路和团队其他同学对齐一下信息
...
```text
早上：
1、和测试分析tce3100支持告警等级出现多次告警的情况，可能是时序问题导致
   需要分析同步器触发时间和amp失效时间、和adp触发告警时间，三者对比；如果amp失效时间 < 同步器同步时间 < adp触发时间，那么就是时序问题
2、M11环境查看支持不同时区的问题，修改了deploy的yaml配置后，启动pod失败了

下午：
1、分析动态时区可能影响点，adp、amp、consumption、domain_policy等等、查看amp如何连接的mysql
   golang连接mysql最终都会使用go-mysql-driver来连接mysql，可以在这里看看连接信息
2、测试验证支持告警等级的问题
```
...
```text
早上：
1、确认tce3.10.11的console中是否有写死时区的 -- 查看后，没有写死时区的
2、验证amp动态时区的发送告警信息

下午:
1、处理mr中的check问题
2、处理mr中的代码冲突问题、和流水线问题
